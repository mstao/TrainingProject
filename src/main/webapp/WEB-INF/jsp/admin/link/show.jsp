...
<section class="dx-list-default">
	<div class="dx-operation-bar">
		<form class="layui-form">
			<div class="layui-form-item">
				<div class="layui-input-inline"> <input   placeholder="请输入链接名称" autocomplete="off" class="layui-input link-input"> </div>
			</div>
	...
	<table class="layui-table link-table">
		<thead>
			<tr>
		...
		<c:forEach items="${linkList}" var="list"> 
			<tr>
				...
				<td class="table-operation-box"> 
				 <span class="table-operation update-link-btn" title="编辑"><i class="layui-icon">&#xe642;</i>   </span>
...
<!--S 修改页面 -->
<div class="update-link update-role">
<br>
<input type="hidden" class="hide-id"/>
<div class="add-link-input add-role-input">
<label>链接名称</label><br>
<input type="text"  placeholder="请填入链接名称" class="update-name  update-property"><br><br>
<label>链接url</label><br>
<input type="text"  placeholder="请填入链接url" class="update-url update-property">
</div>
<!-- <div class="active">
...
layui.use(['layer','element','form'], function() {
	...
	$('#add').on('click', function(){
		 layer.open({
		      ...
		      //  skin: 'layui-layer-lan',
		        ...
		        maxmin: true,
		        area: ['600px', '300px'],
		        content: CTP_PATH+'/admin/link/showAddLink',
          ...
          layer.confirm('确定删除所选择的记录？', {
  			  btn: ['确定','取消'] 
  				...
  				$.ajax({
					type:'post',
					dataType:'json',
					url:CTP_PATH+"/admin/link/delete",
					data:{"ids":ids},
					...
					success:function(data){
						...
						if(data==1){
							...
							window.location.href=CTP_PATH+"/admin/link/show";
	...
	$(".find").bind("click",function(){
		...
		if(token.replace(/\s/g , '')!="" ){
			...
			$.ajax({
				type:'post',
				dataType:'json',
				url:CTP_PATH+'/admin/link/findLink',
				data:{"token":token},
				...
				success:function(data){
			        ...
			        var xhtml="";
			        xhtml+="<thead><tr><th><input type='checkbox' class='checked_input'/></th><th>链接名称</th><th>链接URL</th><th>操作</th></tr></thead>";
			        ...
			        if(data.length>0){
			        	...
			        	xhtml+="<tr><td colspan='4' style='background:#F2DEDE; color:#444444;'>没有找到链接信息</td></tr>";
			        }
			        
			        xhtml+="</tbody>";
			        $(".link-table").html(xhtml);
	...
	//更新栏目
	$(document).on("click",".update-link-btn",function(){
	/* $(".update-link-btn").bind("click",function(){ */
		var r_td = $(this).parent().parent().children()
		//获取id
		var id = r_td.eq(0).attr("data-id");
		var name = r_td.eq(1).text();
		var url = r_td.eq(2).text();
		...
		layer.open({
		    ...
		    yes:function(){
				...
				var sname = $(".update-name").val();
				var surl = $(".update-url").val();
			
				 $.ajax({
					type:'post',
					dataType:'text',
					url:CTP_PATH+"/admin/link/update",
					data:{"id":id,"linkName":sname,"linkUrl":surl},
